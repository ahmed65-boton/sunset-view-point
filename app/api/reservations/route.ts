...
export async function POST(req: Request) {
  ...
  try {
    ...
    reservationRef = db.collection("reservations").doc(); // create ID now (so we can update after)
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      return { ok: true as const, reservationId: reservationRef!.id };
    });
    ...
    // ---- Send email AFTER booking succeeds ----
    try {
      await sendEmailViaResend({
        ...
        html: `
          ...
          <ul>
            <li><b>Date:</b> ${dateDisplay}</li>
            <li><b>Time:</b> ${input.time}</li>
            <li><b>Guests:</b> ${input.guests}</li>
          </ul>
          <p><b>Booking ID:</b> ${result.reservationId}</p>
          <p>See you soon!</p>
        `,
      });
      ...
      await reservationRef!.update({
        ...
        "confirmation.lastAttemptAt": FieldValue.serverTimestamp(),
      });
    } catch (e: any) {
      await reservationRef!.update({
        "confirmation.emailSent": false,
        "confirmation.emailError": String(e?.message ?? e),
        "confirmation.lastAttemptAt": FieldValue.serverTimestamp(),
      });
    }
