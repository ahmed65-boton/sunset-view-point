...
import type { MenuCategory, SelectedMenuItems } from "@/lib/menu";
...
export default function BookingPage() {
  ...
  const [menuCategories, setMenuCategories] = useState<MenuCategory[] | undefined>(undefined);

  useEffect(() => {
    let isMounted = true;

    fetch("/api/menu")
      .then((res) => (res.ok ? res.json() : Promise.reject(new Error("Menu load failed"))))
      .then((data) => {
      ...
      .catch((err) => {
        console.warn("[BookingPage] Using fallback menu:", err);
      });
          ...
          <div className="svp-container">
            <div className="grid grid-cols-1 gap-8 xl:grid-cols-[minmax(0,1.05fr)_minmax(26rem,0.95fr)] xl:items-start">
              <div data-aos="fade-right" data-aos-delay="150">
                <MenuSection selectedItems={selectedItems} onSelectedItemsChange={setSelectedItems} menuCategories={menuCategories} />
              </div>
              <div className="xl:sticky xl:top-24" data-aos="fade-left" data-aos-delay="220">
                <BookingForm selectedItems={selectedItems} onClearOrder={() => setSelectedItems({})} menuCategories={menuCategories} />
              </div>
            </div>
          </div>
        </section>
