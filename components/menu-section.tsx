...
export function MenuSection({ selectedItems, onSelectedItemsChange, menuCategories }: MenuSectionProps) {
  const categories = menuCategories?.length ? menuCategories : fallbackMenuCategories;
  ...
  const subtotal = getOrderSubtotal(selectedItems, categories);
  ...
  const activeCategoryCount = categories.find((category) => category.name === activeCategory)?.items.length ?? 0;
